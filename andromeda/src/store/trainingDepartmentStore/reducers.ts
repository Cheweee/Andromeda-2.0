...
export function trainingDepartmentReducer(prevState: TrainingDepartmentState = initialState, action: TrainingDepartmentsActions): TrainingDepartmentState {
    switch (action.type) {
        ...
        case ActionType.createTrainingDepartmentSuccess: {
            ...
            const state: TrainingDepartmentsState = { trainingDepartmentsLoading: false, trainingDepartments: prevState.trainingDepartments.concat(action.department) };
            return { ...prevState, ...state };
        }
        case ActionType.updateTrainingDepartmentSuccess: {
            if (prevState.trainingDepartmentsLoading === true) return prevState;

            const updated = prevState.trainingDepartments.map(o => o.id == action.department.id ? action.department : o);
            const state: TrainingDepartmentsState = { trainingDepartmentsLoading: false, trainingDepartments: updated };
            return { ...prevState, ...state };
        }
